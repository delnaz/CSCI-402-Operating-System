...
special_file_read(vnode_t *file, off_t offset, void *buf, size_t count)
{
	...
	/*----------------------------------------------------------------------------------*/
       int bytes_read = 0;
       KASSERT(file);
       dbg(DBG_PRINT,"\n(GRADING2A 1.a)  Pointer to file not NULL file = %ld \n", (long)file->vn_vno);
      
       KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
       dbg(DBG_PRINT,"\n(GRADING2A 1.a)  It is a character special file or block special file \n");
	if(S_ISCHR(file->vn_mode))
          {
            KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->read);
            dbg(DBG_PRINT,"\n(GRADING2A 1.a)  Corresponding device exists with operations \n");
          }
	...
	bytes_read = file->vn_cdev->cd_ops->read(file->vn_cdev, offset, buf, count);
...
special_file_write(vnode_t *file, off_t offset, const void *buf, size_t count)
{
	...
	/*-----------------------------------------------------------------------------*/
       int bytes_written = 0;
       KASSERT(file);
       dbg(DBG_PRINT,"\n(GRADING2A 1.b)  Pointer to file not NULL file = %ld", (long)file->vn_vno);
      
       KASSERT((S_ISCHR(file->vn_mode) || S_ISBLK(file->vn_mode)));
       dbg(DBG_PRINT,"\n(GRADING2A 1.b)  It is a character special file or block special file ");

	if(S_ISCHR(file->vn_mode))
          {
            KASSERT(file->vn_cdev && file->vn_cdev->cd_ops && file->vn_cdev->cd_ops->write);
            dbg(DBG_PRINT,"\n(GRADING2A 1.b)  Corresponding device exists with operations ");
          }
	if(S_ISBLK(file->vn_mode))
         {
           return -ENOTSUP;
        ...
        bytes_written = file->vn_cdev->cd_ops->write(file->vn_cdev, offset, buf, count);
