...
void *checkRead (int arg1, void *arg2)
{
	int read = 0;
	int fd = 33;
	char *buf = "This is a test buffer";
	size_t nbytes = 10;
	read = do_read(fd, (void *)buf, nbytes);
	if (read < 0)
		dbg(DBG_ERROR,"\nSTUDENT TEST: Read operation was not successful - fd (%d) is out of limit\n", fd);
	return NULL;
}

void *checkWrite (int arg1, void *arg2)
{
	int write = 0;
	int fd = 33;
	char *buf = "This is a test buffer";
	size_t nbytes = 10;
	write = do_write(fd, (const void *)buf, nbytes);
	if (write < 0)
		dbg(DBG_ERROR,"\nSTUDENT TEST: Write operation was not successful - fd (%d) is out of limit\n", fd);
	return NULL;
}

void *checkRename1 (int arg1, void *arg2)
{
	int rename = -1;
	const char *dirname = "/test";
	const char *filename = "/test/file";
	const char *newname = "/test/filenew";

	do_mkdir(dirname);
	do_mknod(filename, S_IFCHR, MKDEVID(1,2));

	dbg(DBG_PRINT,"\nSTUDENT TEST: Rename operation executing\n");
	rename = do_rename(filename, newname);

	if (rename == 0)
	{
		dbg(DBG_PRINT,"\nSTUDENT TEST: Rename operation succeeded - (%s) renamed to (%s)\n", filename, newname);
		return NULL;
	}
	else
	{
		dbg(DBG_ERROR,"\nSTUDENT TEST: Rename operation failed\n");
		return NULL;
	}
}

void *checkRename2 (int arg1, void *arg2)
{
	int rename;
	const char *dirname = "/test";
	const char *filename1 = "/test/file1";
	const char *filename2 = "/test/file2";
	const char *newfilename = "/test/filenew";

	do_mkdir(dirname);
	do_mknod(filename1, S_IFCHR, MKDEVID(1,3));

	dbg(DBG_PRINT,"\nSTUDENT TEST: Rename operation executing\n");
	rename = do_rename(filename2, newfilename);
...
void *checkRename3 (int arg1, void *arg2)
{
	int rename;
	const char *oldname = "/test";
	const char *newname = "/testnew";
	...
	int vfsTest(kshell_t *kshell, int argc, char **argv)
        {
        	KASSERT(kshell != NULL);
        	dbg(DBG_INIT, "(GRADING2B): vfs_test() is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
		sched_make_runnable(kthread_create(proc_create("vfsTestProc"), vfstest_main, 1, NULL));
		int status = 0;while(do_waitpid(-1,0,&status) > 0);
		/* vfstest_main(1, NULL); */
            	return 0;
        }
		...
		/**/
        	...
        	argc, (unsigned int)argv);
		sched_make_runnable(kthread_create(proc_create("faberProcess"), testproc, 0, NULL));
		int status = 0;while(do_waitpid(-1,0,&status) > 0);
        ...
        int sunghanTest(kshell_t *kshell, int argc, char **argv)
        {
        	...
        	argc, (unsigned int)argv);
		sched_make_runnable(kthread_create(proc_create("sunghanProcess"), sunghan_test, 0, NULL));
		int status = 0;while(do_waitpid(-1,0,&status) > 0);
		/*sunghan_test(0, NULL);*/
            	return 0;
        }
	...
	int readTestFun(kshell_t *kshell, int argc, char **argv)
	{
		KASSERT(kshell != NULL);
        	dbg(DBG_INIT, "(GRADING2C 1): checkRead is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
		checkRead(0, NULL);
            	return 0;
	}

	int writeTestFun(kshell_t *kshell, int argc, char **argv)
	{
		KASSERT(kshell != NULL);
        	dbg(DBG_INIT, "(GRADING2C 2): checkWrite is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
		checkWrite(0, NULL);
            	return 0;
	}

	int renameValid(kshell_t *kshell, int argc, char **argv)
	{
		KASSERT(kshell != NULL);
        	dbg(DBG_INIT, "(GRADING2C 3): checkRename1 is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
		checkRename1(0, NULL);
            	return 0;
	}

	int renameInvalid(kshell_t *kshell, int argc, char **argv)
	{
		KASSERT(kshell != NULL);
        	dbg(DBG_INIT, "(GRADING2C 4): checkRename2 is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
		checkRename2(0, NULL);
            	return 0;
	}

	int renamedir(kshell_t *kshell, int argc, char **argv)
	{
		KASSERT(kshell != NULL);
        	dbg(DBG_INIT, "(GRADING2C 5): checkRename3 is invoked, argc = %d, argv = 0x%08x\n",
        	argc, (unsigned int)argv);
		checkRename3(0, NULL);
            	return 0;
	}
...
initproc_run(int arg1, void *arg2)
{
		...
		kshell_add_command("vfstest", vfsTest, "Runs VFS test");
		kshell_add_command("readTest", readTestFun, "Read operation tests edge case on filedescriptor fd");
		kshell_add_command("writeTest", writeTestFun, "Write operation tests edge case on filedescriptor fd");
		/* kshell_add_command("renameValidTest", renameValid, "Rename existing file"); */
		kshell_add_command("renameInvalidTest", renameInvalid, "Rename non-existent file");
		kshell_add_command("renameDirTest", renamedir, "Rename directory");
